...
class _InventoryPageState extends State<InventoryPage> {
  ...
  Future<void> fetchItems() async {
    const url = 'http://10.0.2.2/minoriiproject/sinventory.php';
    try {
          ...
          setState(() {
            items = List<Map<String, dynamic>>.from(data['items']).map((item) {
              return {
                ...
                'quantity':
                    int.parse(item['quantity'].toString()), // Ensure conversion
                'max_quantity': int.parse(
                    item['max_quantity'].toString()), // Ensure conversion
              };
            }).toList();
          });
        } else {
  ...
  void showItemDialog({Map<String, dynamic>? item}) {
    ...
    itemQuantityController.text =
        item != null ? item['quantity'].toString() : ''; // Convert to String
    maxQuantityController.text = item != null
                    ...
                    ...items.map((item) {
                      ...
                      return TableRow(
                        decoration: BoxDecoration(
                          color: isLowStock
                              ? const Color.fromARGB(255, 233, 34, 54)
                              : null,
                        ),
                        children: [
                          Padding(
